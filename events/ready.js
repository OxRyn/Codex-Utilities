    ...
    new Promise(async (resolve, reject) => {
      ...
      client.config.leaveOnEmptyCooldown =
        typeof process.env.LEAVE_ON_EMPTY_COOLDOWN === "undefined"
          ...
          : Number(process.env.LEAVE_ON_EMPTY_COOLDOWN)
            ? process.env.LEAVE_ON_EMPTY_COOLDOWN
            : client.config.leaveOnEmptyCooldown;
      ...
      client.config.leaveOnEndCooldown =
        typeof process.env.LEAVE_ON_END_COOLDOWN === "undefined"
          ...
          : Number(process.env.LEAVE_ON_END_COOLDOWN)
            ? process.env.LEAVE_ON_END_COOLDOWN
            : client.config.leaveOnEndCooldown;
      ...
      client.config.leaveOnStopCooldown =
        typeof process.env.LEAVE_ON_STOP_COOLDOWN === "undefined"
          ...
          : Number(process.env.LEAVE_ON_STOP_COOLDOWN)
            ? process.env.LEAVE_ON_STOP_COOLDOWN
            : client.config.leaveOnStopCooldown;
      ...
      client.config.defaultVolume =
        typeof process.env.DEFAULT_VOLUME === "undefined"
          ? client.config.defaultVolume
          : Number(process.env.DEFAULT_VOLUME <= 100) &&
              Number(process.env.DEFAULT_VOLUME >= 0)
        ...
        await fetch(
            ...
            if (search.status == 401) {
              console.log(
                `${ConsoleStyles.error(`[E]`)} ${ConsoleStyles.string(
                  `Plex configuration is invalid. Disabling Plex feature... Your Plex Authentication token is not valid.`,
                )}`,
              );
              ...
              console.log(
                `${ConsoleStyles.error(`[E]`)} ${ConsoleStyles.string(
                  `Plex configuration is invalid. Disabling Plex feature... Generic error.`,
                )}`,
              );
              client.config.enablePlex = false;
            }
          ...
          .catch((err) => {
            if (controller.signal.aborted) {
              console.log(
                `${ConsoleStyles.error(`[E]`)} ${ConsoleStyles.string(
                  `Plex configuration is invalid. Disabling Plex feature... Read more in the trace below:\n${controller.signal.reason}`,
                )}`,
              );
            } else {
              console.log(
      ...
      console.log(
        `${ConsoleStyles.success(`[S]`)} ${ConsoleStyles.string(
          `Loading successful. Core of the bot is ready!`,
        )}`,
      );
    });

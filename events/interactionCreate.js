  ...
  async execute(interaction) {
      ...
      if (timestamp.has(interaction.user.id)) {
        ...
        if (curtime < expiration) {
          ...
          return interaction.reply({
            content: `⏱️ | Cooldown Alert: Please wait **${Match.ceil(
              timeleft
            )}** more seconds before using the **/${
              command.data.name
            }** command again!`,
            ephemeral: true,
          });
        ...
        fs.readdirSync("./commands/").forEach((dir) => {
          ...
          commands.map((command) => {
            ...
            if (dir == "configuration" || dir == "utilities") {
              ...
              if (
                file.data.options.length == 0 ||
                file.data.options[0].type != null
              ) {
                cmds.push({
                  name: dir,
                  commands: {
        ...
        if (value && value !== "home") {
          ...
          embed.setTitle(
            `Help Menu - ${
              ...
              emojis[categories[value][0].name]
                ? emojis[categories[value][0].name]
                : ""
            }`
          );
          ...
          if (getchannel) {
            ...
            if (
              ...
              getchannel.messages.fetch(interaction.message.id).then(
                async (msg) =>
                  await msg.edit({
                    embeds: [embed],
                    components: [row],
    ...
    //Check for button interactions
      ...
      if (interaction.customId == "queue-pageleft") {
        ...
        const queueembed = new MessageEmbed()
          ...
          .addField(
            "Now Playing ▶️",
            `**${currentMusic.title}** ${
              currentMusic.queryType != "arbitrary"
                ? `([Link](${currentMusic.url}))`
                : ""
            }`
          )
          .setTimestamp()
      ...
      if (interaction.customId == "np-volumeup") {
        ...
        if (totalVol > 100)
          return interaction.reply({
            content: "❌ | The volume is already set to the max!",
            ephemeral: true,
          });
      ...
      if (interaction.customId == "np-loop") {
        ...
        if (queue.repeatMode === QueueRepeatMode.TRACK) {
          ...
          const loopembed = new EmbedBuilder()
            ...
            .setTitle(mode)
            .setDescription(
              `The loop mode has been set to the **current track**!`
            )
            .setTimestamp()
            .setFooter({ text: `Requested by: ${interaction.user.tag}` });
